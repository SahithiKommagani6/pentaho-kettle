...
public class RepositorySessionTimeoutHandler implements InvocationHandler {
  ...
  private Object handleSessionExpiry( Method method, Object[] args ) throws Throwable {
    Spoon spoon = Spoon.getInstance();

    // Set relogin flag BEFORE disconnecting to prevent "Close Files" popup
    ...
    if ( loginSuccessful && spoon.getRepository() != null && spoon.getRepository().isConnected() ) {
      return reconnectAndRetry( spoon, method, args );
    }
  ...
  private Object reconnectAndRetry( Spoon spoon, Method method, Object[] args )
      throws SessionRecoveryRetryException {
    this.repository = (ReconnectableRepository) spoon.getRepository();
    try {
