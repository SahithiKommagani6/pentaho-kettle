...
public class SessionTimeoutHandler {
   ...
   */
  private Object performLoginAndReinvoke( Object objectToHandle, Method method, Object[] args ) throws Throwable {
    synchronized ( this ) {
      if ( !needToLogin.get() ) {
        return null;
      }
  ...
  boolean showLoginScreen( RepositoryConnectController repositoryConnectController ) {
    ...
    if ( wasBrowserAuth && serverUrl != null ) {
  ...
  private boolean tryBrowserReauth( String serverUrl, RepositoryMeta repositoryMeta,
    ...
    try {
      ...
      SessionInfo sessionInfo = authService.authenticate( serverUrl ).get();
